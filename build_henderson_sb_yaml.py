...
def build_network():
    ...
    # Filter to main tracks (SG, 1, 2)
    df = df[df['TrackNumber'].isin(['SG', '1', '2'])].copy()
    print(f"  After main track filter: {len(df)} rows")

    # Sort by milepost and drop parallel duplicate tracks to form single route
    df = df.sort_values(['beg_mp', 'TrackNumber']).drop_duplicates(subset=['beg_mp'], keep='first').reset_index(drop=True)
