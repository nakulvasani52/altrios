...
OUT_DIR = Path("results/henderson_full_sim/EDA")
...
# Remove dummy buffers to focus on main corridor
df_main = df[(df['milepost'] >= df['milepost'].min() + 31) & (df['milepost'] <= df['milepost'].max())].copy()
if len(df_main) == 0:
    df_main = df
