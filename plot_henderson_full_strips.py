...
def plot_full_4panel(sim_df, geo_df):
    """4-panel strip chart:
    ...
    ax1.set_title(
        f"Henderson Subdivision NB: Infrastructure Geometry (MP {MP_MIN:.0f}–{MP_MAX:.0f})",
...
def plot_geometry_speed(sim_df, geo_df):
    merged = sim_df.merge(geo_df, on='mp_group', how='left').fillna(0)
    merged = merged.sort_values('mp_group')
    x = merged['mp_group'].values
...
def plot_resistance_stacked(sim_df):
    ...
    ax.set_title(
        f"Henderson NB: Resistance Force Breakdown (MP {MP_MIN:.0f}–{MP_MAX:.0f})",
...
def plot_demand(sim_df):
    ...
    ax.set_title(
        f"Henderson NB: Track Longitudinal Demand (MP {MP_MIN:.0f}–{MP_MAX:.0f})",
        fontsize=16, fontweight="bold")
    ax.set_xlabel("Milepost", fontweight="bold", fontsize=14)
...
if __name__ == "__main__":
    ...
    MP_MAX = min(MP_MAX, sim_df['mp_group'].max() + 0.5)

    plot_full_4panel(sim_df, geo_df)
    plot_geometry_speed(sim_df, geo_df)
    plot_resistance_stacked(sim_df)
    plot_demand(sim_df)
